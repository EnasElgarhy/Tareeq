...
import { z } from "zod";
import {
  type AudioRegenSummary,
  regenerateStaleAudioForVersion,
} from "@/lib/admin/audio-generation";
...
export async function publishVersion(
  ...
  // Best-effort: publish already succeeded above regardless of this. Only
  // regenerates clips whose title text actually changed (source_text_hash
  // mismatch) or that reuse an identical clip from a prior version — keeps
  // ElevenLabs usage down instead of re-generating all ~50 questions × 2
  // locales on every publish.
  try {
    return await regenerateStaleAudioForVersion(versionId);
  } catch (err) {
    console.error(`Audio regen sweep failed for version ${versionId}:`, err);
    return { generated: 0, reused: 0, upToDate: 0, skipped: 0, failed: 0 };
  }
}
