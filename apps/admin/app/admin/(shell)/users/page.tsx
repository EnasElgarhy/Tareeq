...
import { listEmailInvites } from "@/lib/admin/access/actions";
import { requireAdmin } from "@/lib/auth/require-admin";
import { hasPermission } from "@/lib/admin/team/permissions";
import { InviteStudentSheet } from "@/components/admin/users/InviteStudentSheet";
...
function Metric({
  ...
  return (
    <Card className="p-4">
      <p className="text-[11px] font-bold uppercase tracking-wider text-adm-ink-muted">
        {label}
      </p>
      <p className="mt-1 text-[26px] font-black leading-none text-adm-ink">
        {value}
      </p>
      {sub ? (
        <p className="mt-1 text-[12px] text-adm-ink-faint">{sub}</p>
      ) : null}
    </Card>
  );
}
...
const FILTER_SELECTS: Array<{
  ...
  },
  {
    ...
    options: [
      ...
      ["compass_completed", "Compass done"],
      ["multiple_completed", "Multiple done"],
    ],
  },
  {
    name: "kai",
...
export default async function UsersPage({
  ...
  const { rows, total, summary } = await getUsersList(
    filter,
    sort,
    pagination,
    now,
  ...
  // Email invites need `users.manage`; the list itself stays readable with
  // `users.view`. A lookup failure hides the card, never the page.
  const { role } = await requireAdmin();
  const canGrant = hasPermission(role, "users.manage");
  let emailInvites: Awaited<ReturnType<typeof listEmailInvites>> = [];
  if (canGrant) {
    try {
      emailInvites = await listEmailInvites();
    } catch {
      emailInvites = [];
    }
  }
  ...
  return (
    <>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
        ...
        <Metric
          label="Compass rate"
          value={pct(summary.compassCompletionRate)}
          sub="of those who started"
        />
        <Metric
          label="Kai adoption"
          value={pct(summary.kaiAdoptionRate)}
          sub="of all users"
        />
      ...
      {canGrant ? (
        <div className="mb-4 flex justify-end">
          <InviteStudentSheet invites={emailInvites} />
        </div>
      ) : null}
      ...
      >
        <label className="flex flex-col gap-1">
          <span className="text-[10px] font-bold uppercase tracking-wider text-adm-ink-muted">
            Search
          </span>
          <input
            type="search"
        ...
        {FILTER_SELECTS.map((s) => (
          <label key={s.name} className="flex flex-col gap-1">
            ...
            <select
              name={s.name}
              defaultValue={
                (filter as unknown as Record<string, string>)[s.name]
              }
              className={selectCls}
            >
              {s.options.map(([v, l]) => (
        ...
        {!isDefaultFilter(filter) || sort !== "registered_desc" ? (
          <Link
            href="/admin/users"
            className="px-2 py-1.5 text-[12px] font-semibold text-adm-violet hover:text-adm-deep"
          >
            Clear
          </Link>
        ) : null}
      ...
      {summary.totalUsers === 0 ? (
        <EmptyState
          ...
          description="Registered students will appear here once people sign up."
        />
      ) : rows.length === 0 ? (
        <EmptyState
          title="No matches"
          description="No students match these filters. Try clearing them."
        />
      ) : (
        <>
          <Table>
                ...
                <Tr key={r.id}>
                  <Td>
                    <Link
                      ...
                      className="flex items-center gap-2.5 font-semibold text-adm-ink hover:text-adm-violet"
                    >
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="block truncate text-[11px] font-normal text-adm-ink-faint">
                          {r.email ?? "no email"}
                        </span>
                      </span>
                    </Link>
                  </Td>
                  <Td className="uppercase text-adm-ink-muted">
                    {r.language ?? "—"}
                  </Td>
                  <Td className="text-adm-ink-soft">{r.country ?? "—"}</Td>
                  <Td>
                    <StatusBadge
                      status={r.status === "deleted" ? "archived" : "published"}
                    />
                  </Td>
                  ...
                  <Td>
                    <Link
                      href={`/admin/users/${r.id}`}
                      className="text-adm-violet hover:text-adm-deep"
          ...
          <div className="mt-3 flex items-center justify-between text-[12px] text-adm-ink-muted">
            <span>
              {total} student{total === 1 ? "" : "s"} · page {pagination.page}{" "}
              of {totalPages}
            </span>
            <span className="flex gap-2">
              {pagination.page > 1 ? (
                <Link
                  ...
                  className="rounded-adm-md border border-adm-line px-2.5 py-1 font-semibold text-adm-ink-soft hover:bg-adm-sand"
                >
                  ← Prev
                </Link>
              ) : null}
