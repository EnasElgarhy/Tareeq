 ...
 *
 * Choice types (single / binary / dropdown): type the question, Enter. Type an
 * option; `@` pops the cluster picker, `*2` sets weight. Enter stages the
 * option. Enter on an empty input (or ⌘/Ctrl+Enter) saves and the loop
 * restarts. Backspace on an empty input pulls the last option back in.
 * Free-text type: no options — Enter on the question saves.
 *
 * Save uses the EXISTING addQuestion action. NOTE: option `weight` is captured
 * in the UI but the current question_options schema has no weight column, so it
 * isn't persisted yet (would need a migration + scoring change).
 */
...
export function QuestionBuilder({ versionId }: { versionId: string }) {
  ...
  async function saveQuestion() {
    if (!qText.trim()) {
      setError(
        withOptions
          ? "Write the question first — then Enter to start adding options."
          : "Write the question, then press Enter to save.",
      );
      questionRef.current?.focus();
      return;
    }
    if (withOptions && staged.length < 2) {
  ...
  /* ---- keyboard ---- */
  function onQuestionKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key !== "Enter") return;
    e.preventDefault();
    if (withOptions) optionRef.current?.focus();
  ...
  const selectCls =
    "rounded-adm-sm border border-adm-line-strong bg-adm-card px-2 py-1 text-[12px] font-semibold text-adm-ink focus:border-adm-violet focus:outline-none focus:ring-2 focus:ring-adm-violet/25";
  const kbd =
  ...
  return (
    <Card tone="tinted" className="w-full border-dashed border-adm-line-strong p-5">
      ...
      <p className="mb-4 text-xs text-adm-ink-muted">
        {withOptions ? (
          <>
            ...
            question → options → save. <span className={kbd}>@</span> tags a
            cluster, <span className={kbd}>*2</span> sets weight.
          </>
        ) : (
          <>
            Free-text answers — type the question and press{" "}
            <span className={kbd}>Enter</span> to save.
          </>
        )}
      </p>
      ...
      <input
        ...
        placeholder={
          withOptions
            ...
            : "Type the question, press Enter to save…"
        }
      ...
      {withOptions ? (
              ...
              >
                {matches.map((c, i) => (
                  ...
                  >
                    ...
                    {c.name}
                    {i === hl && (
                      <span className="ml-auto text-[11px] text-adm-ink-muted">
                        ↵
                      </span>
                    )}
                  </li>
        ...
        <p className="mt-3 rounded-adm-md bg-adm-card/60 px-4 py-3 text-[13px] text-adm-ink-muted">
          Free-text question — respondents type their own answer. No preset
          options.
        </p>
      )}
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        {error ? (
          ...
          <p className="text-xs text-adm-ink-muted" aria-live="polite">
            {!withOptions
              ? "Free-text — press Enter on the question to save."
              : staged.length === 0
        ...
        <Button
          ...
          loading={saving}
          disabled={!qText.trim() || (withOptions && staged.length < 2)}
          className="shrink-0"
        >
          Save question ⌘↵
        </Button>
