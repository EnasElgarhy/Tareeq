...
export function VersionActions({
  ...
  async function doPublish() {
    ...
    setBusy("publish");
    try {
      const audio = await publishVersion(versionId);
      const audioNote =
        audio.generated + audio.reused > 0
          ? ` Audio: ${audio.generated} regenerated, ${audio.reused} reused, ${audio.upToDate} unchanged${audio.failed ? `, ${audio.failed} failed` : ""}.`
          : "";
      toast(
        "success",
        `Published — this version is now live for students.${audioNote}`,
      );
      router.refresh();
    } catch (e) {
