...
import {
  getSubjectReason,
  youtubeSearchUrl,
} from "@/lib/results/report-helpers";
...
export function CompassReport({
  ...
  return (
    <div className="grid gap-3">
      <ExpandableSection
        icon={<ArchetypeMarkIcon size={18} />}
        title={t("profile.compass.why_title")}
        defaultOpen
      >
        <p>{report.summary}</p>
      </ExpandableSection>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[13.5px] font-black text-[color:var(--day-ink,#2a2118)]">
              ...
              {t(getConfidenceLabelKey(report.score.confidenceLabel))}
            </p>
          </div>
        </div>
          ...
          {rankedClusters.map(([code, score], index) => (
            <div key={code} className="grid gap-1">
              <div className="flex items-center justify-between gap-2 text-[12px]">
                ...
                <span className="tabular-nums text-[color:var(--day-ink-3,#675d4e)]">
                  {score}
                </span>
              </div>
              <span className="block h-1.5 overflow-hidden rounded-full bg-[color:var(--day-inset,#efe7da)]">
                <span
                  ...
                  style={{
                    background:
                      index === 0 ? "var(--grad-warm)" : "rgba(43,36,28,0.18)",
      ...
      <div className="grid gap-2">
        ...
        {report.careerExamples.slice(0, 6).map((career, index) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[13px] font-black text-[color:var(--day-ink,#2a2118)]">
                {career}
              </p>
              <a
      ...
      <TagPanel
        icon={<MajorIcon size={18} />}
        title={t("profile.compass.majors_title")}
      ...
      <div className="rounded-[20px] border border-[color:var(--day-line,rgba(43,36,28,0.1))] bg-[color:var(--day-card,#fffcf6)] p-3.5 shadow-[0_8px_20px_rgba(43,36,28,0.05)]">
        <div className="flex items-center gap-2.5">
          ...
          <p className="text-[13px] font-black text-[color:var(--day-ink,#2a2118)]">
            {t("profile.compass.subjects_title")}
          </p>
        </div>
        <div className="mt-2.5 grid gap-1.5">
          {report.highSchoolSubjects.slice(0, 6).map((subject) => (
            <details
              key={subject}
              className="group rounded-[14px] border border-[color:var(--day-line,rgba(43,36,28,0.1))] bg-[color:var(--day-inset,#efe7da)] px-3 py-2"
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-2 [&::-webkit-details-marker]:hidden">
                <span className="text-[12.5px] font-bold text-[color:var(--day-ink,#2a2118)]">
                  {subject}
                </span>
                <ChevronDown
                  size={14}
                  className="shrink-0 text-[color:var(--day-ink-3,#675d4e)] transition group-open:rotate-180"
                />
              </summary>
      ...
      <TagPanel
        ...
        title={t("profile.compass.non_obvious_title")}
        items={report.nonObviousPaths}
      />
      <ExpandableSection
        icon={<GoalIcon size={18} />}
        title={t("profile.compass.landscape_title")}
      >
        <p>{report.careerLandscape}</p>
      </ExpandableSection>
      <ExpandableSection
        icon={<MajorIcon size={18} />}
        title={t("profile.compass.academic_title")}
      >
        <p>{report.academicPath}</p>
      </ExpandableSection>
      <ExpandableSection
        icon={<ResearchIcon size={18} />}
        title={t("profile.compass.reality_title")}
      >
        <p>{report.realityCheck}</p>
      </ExpandableSection>
      <ExpandableSection
        icon={<ArchetypeMarkIcon size={18} />}
        title={t("profile.compass.integration_title")}
      >
        <p>{report.integration}</p>
      </ExpandableSection>
      <ExpandableSection
        icon={<ActionPlanIcon size={18} />}
        title={t("profile.compass.next_steps_title")}
      >
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-[12.5px] font-black text-[color:var(--day-ink,#2a2118)]">
            {t("profile.compass.ask_kai_title")}
          </span>
          <span className="block text-[11px] text-[color:var(--day-ink-3,#675d4e)]">
            {t("profile.compass.ask_kai_subtitle")}
          </span>
        </span>
      </a>
    </div>
  ...
  return (
    ...
    >
      <summary className="flex cursor-pointer list-none items-center gap-2.5 [&::-webkit-details-marker]:hidden">
        ...
        <span className="min-w-0 flex-1 text-[13px] font-black text-[color:var(--day-ink,#2a2118)]">
          {title}
        </span>
        <ChevronDown
          size={16}
      ...
      <div className="mt-3 border-t border-[color:var(--day-line,rgba(43,36,28,0.1))] pt-3 text-[12.5px] leading-relaxed text-[color:var(--day-ink-2,#5c5142)]">
        {children}
      </div>
    </details>
  );
...
function TagPanel({
  ...
  return (
    <div className="rounded-[20px] border border-[color:var(--day-line,rgba(43,36,28,0.1))] bg-[color:var(--day-card,#fffcf6)] p-3.5 shadow-[0_8px_20px_rgba(43,36,28,0.05)]">
      <div className="flex items-center gap-2.5">
        <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-gold/12">
          {icon}
        </span>
        <p className="text-[13px] font-black text-[color:var(--day-ink,#2a2118)]">
          {title}
